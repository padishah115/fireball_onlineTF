...
class DigicamImageManager(ImageOperationsManager):
    ...
    def _get_moments(self):
            """Calculates the first and second moments of pixel intensity for the image."""

            thresh = 0.2
            max_intensity = np.max(self.shot_data["DATA"])
            img = self.shot_data["DATA"]

            # Make sure that we have a 2D image 
            if img.shape == None or img.shape[0] == 0 or img.shape[1] == 1:
                raise ValueError(f"Error: image shape is {img.shape}.")
            ...
            if total_counts == 0:
                raise ValueError(f"Error: calculating moment of image with 0 pixel counts.")
...
class AndorImageManager(ImageOperationsManager):
    ...
    def plot(self, step:int=200, norm=False):
        ...
        # Initialise the image and the y-axis for the data. Note that the X axis is essentially meaningless.
        img = self.shot_data["DATA"]
        pixels_y = self.shot_data["Y"]
        ...
        # Initialise normalization factor
        normalization_factor = np.max(img) if norm else 1.0
        img = np.divide(img, normalization_factor)

        # Wrap all the analysis in here in case we just want to speed things up
        if not self.input["PLOT_ONLY"]:
            # NEED TO USE NANSUM TO ENSURE THAT THE IMAGE LINEOUT IS DONE CORRECTLY
            
            # Sum along x-axis to produce fourier transformation.
            intensities = np.nansum(img, axis=1)

            # CHECK WHETHER WE HAVE STD DEVIATION INFORMATION
            if self.std_data is not None:
                # Upper and lower bound for intensity lineouts (+- 1 std dev)
                upper_intensities = np.multiply(np.add(np.nansum(self.std_data["DATA"], axis=1), intensities), normalization_factor**-1)
                lower_intensities = np.multiply(np.subtract(intensities, np.nansum(self.std_data["DATA"], axis=1)), normalization_factor**-1)
            

        
            # Initialise figure for image plot and lineout plot.
            ...
            #INTEGRATED IMAGE
            axs[1].plot(intensities, pixels_x)
...
class OrcaImageManager(ImageOperationsManager):
    ...
    def plot(self, step:int=100, norm=False):
        ...
        # WRAP ALL THE ANALYSIS IN HERE SO THAT WE CAN SKIP IT IF WE WANT TO SPEED THINGS UP
        if not self.input["PLOT_ONLY"]:
            # Check for std_data
            if self.std_data is not None:
            ...
            axs.set_xticklabels(x_ticks, rotation=90)
            axs.set_title("Image")
