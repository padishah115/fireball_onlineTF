...
class AndorImageManager(ImageOperationsManager):
    ...
    def plot(self, step:int=200, norm=False):
        ...
        print("Image dtype:", type(img))
        print("Norm factor dtype:", type(normalization_factor))
        ...
        if not self.input["PLOT_ONLY"]:
            ...
            if self.std_data is not None:
                ...
                upper_intensities = np.multiply(np.add(np.sum(self.std_data["DATA"], axis=1), intensities), normalization_factor**-1)
                lower_intensities = np.multiply(np.subtract(intensities, np.sum(self.std_data["DATA"], axis=1)), normalization_factor**-1)
